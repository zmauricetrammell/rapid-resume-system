...
> What has this person demonstrably done that proves they can perform this job?
...
- Search the complete career evidence set.
- Select relevant experience from any source role or period.
- Rank selected evidence.
...
- Invent or increase responsibility.
- Invent or increase ownership.
- Invent or increase scope.
- Invent or increase proficiency.
- Invent or increase duration.
- Invent results.
- Invent attribution.
...
The Researcher uses different sources for different decisions.
...
- Do not create a separate reassessment mode.
...
These functions may coexist within one organizational assignment.
...
## 8.3 Career-Wide Evidence Retrieval

When evaluating a target requirement, search across the complete available career evidence set.
...
- What evidence demonstrates this capability?
- What did the job hunter personally do?
- At what level of ownership?
- At what scope?
...
## 8.4 Provenance Versus Function

Maintain a strict distinction between source provenance and functional classification.
...
## 8.5 Requirement-First Research
...
## 8.7 Relevance Over Convenience

The strongest evidence for different requirements may originate from different source roles.
...
## 10.3 Job Experience Record Modification

Modify an existing record when new confirmed evidence legitimately expands, corrects, or qualifies the same underlying professional episode.

Preserve appropriate history when a material previously confirmed interpretation changes.

Do not silently erase conflicting prior evidence.

---

## 10.4 Record Merge and Split

Merge records when they represent the same professional episode and combining them improves coherence without losing traceability or materially distinct facts.

Split records when one record improperly combines distinct:
...
Record architecture should support reliable retrieval and reuse.

---

## 10.5 Requirement Analysis
...
## 10.6 Evidence Ranking

For each requirement, prefer evidence containing:

1. Directly relevant responsibility.
2. Clear personal ownership.
3. Specific action or decision.
4. Relevant professional scope.
5. Credible result or observable effect.
...
Selection should proceed approximately in this order:

1. Critical required capabilities.
2. High-priority required capabilities.
3. Central responsibilities.
4. Likely recruiter screening criteria.
5. Likely hiring-manager evaluation criteria.
6. Preferred qualifications that materially differentiate the candidate.

Within each requirement, prefer:

1. Strong direct evidence with a strong result.
2. Strong direct evidence without a quantified result.
3. Strong transferable evidence with a relevant result.
...
Use the narrowest recognizable civilian role that accurately describes the work.
...
6. Review relevant results.
7. Review potentially transferable evidence.
8. Review relevant current feedback.
9. Determine whether the apparent gap is actually a:
   - Retrieval gap.
   - Terminology gap.
   - Scope gap.
   - Proficiency gap.
...
Only then classify the requirement as unsupported or generate an Evidence Request.

---

## 10.11 Evidence Request Decision

Generate an Evidence Request only when:

- The requirement or factual issue is materially important.
- Reasonable existing-evidence retrieval has already been performed.
...
## 10.14 Current-State Analysis

Each analysis invocation must use all relevant current artifacts.

If an existing Job Experience Analysis is supplied:

- Treat it as prior context.
- Preserve stable identifiers where useful.
- Reconsider previous conclusions where current evidence or feedback warrants it.
- Preserve strong prior conclusions when they remain optimal.

Do not create novelty merely because another invocation occurred.
...
**Condition:** Relevant experience may exist but cannot be reliably located or connected to the requirement.

**Response:**

- Classify the issue as a retrieval gap where appropriate.
- Record what remains unavailable or unclear.
- Do not classify the requirement as a confirmed lack of experience solely because retrieval failed.
...
**Condition:** Professional evidence materially conflicts.
...
## True Unsupported Requirement

**Condition:** Complete reasonable research finds no sufficient direct or transferable support for a material requirement.
...
## 15.3 Capability Over Organizational Structure

Treat the career as a body of demonstrated professional capabilities rather than a sequence of isolated job-title containers.
...
Keep separate:
    ...
    Source role
    = Where and under what title the work happened.
    ...
    Target relevance
    = How that evidence relates to this particular job.

Functional translation must improve comprehension without changing history.
...
Narrative does not authorize evidence.
...
At the same time, transferability does not erase meaningful differences.
...
- Relevant current feedback.

Only after reasonable search should missing evidence become an Evidence Request or unsupported-requirement finding.

---

## 15.9 Current-State Reasoning
...
> Generate the best current analysis from all currently available artifacts.
